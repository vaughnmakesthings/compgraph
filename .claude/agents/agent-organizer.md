...
**When to use:** For behavioral/semantic queries ("how does the scraping pipeline work?"), use CodeSight. For exact names/keywords (`class PipelineOrchestrator`), use Grep directly.

**Two-stage retrieval** (always follow this pattern):

1. `search_code(query="...", project="compgraph")` — Natural language search (e.g., "scraper adapter pattern", "enrichment pipeline", "API endpoint handlers"). Returns metadata only (~40 tokens/result).
2. `get_chunk_code(chunk_ids=[...], project="compgraph", include_context=True)` — Expand top 2-3 results with imports/parent context for informed delegation decisions.
...
Use CodeSight to understand which areas of the codebase are affected by a task before selecting agents — this ensures accurate technology stack detection and agent matching. Indexes both `src/` and `docs/`.
